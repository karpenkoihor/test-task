...
import {loadCoupons} from "../store/actions/sportActions";
    ...
    useEffect(() => {
        if (!coupons[sportId]) {
            const getCoupons = async () => {
                const fetchedCoupons = await fetchDataFromServer(`/data/coupons/${sportId}.json`);
                if (fetchedCoupons) {
                    dispatch(loadCoupons(sportId, fetchedCoupons));
                }
            };
            getCoupons();
        }
