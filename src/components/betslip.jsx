...
export const Betslip = () => {
  ...
  return (
    <Form onSubmit={(e) => e.preventDefault()}>
      <h3>Betslip</h3>
      {isPlaced ? (
        <Alert variant='success'>Bet has been placed.</Alert>
      ) : (
        selections.length === 0 && <div>Betslip is empty.</div>
      )}
      {selections.map(
        ({
          eventName,
          ...
          date,
          stake,
          selection: { id: selectionId, odds, description },
        }) => (
          <Container key={selectionId} className='mt-2 p-2 bg-light rounded'>
            ...
            <Row>
              ...
              <Col sm='7'>
                <Form.Control
                  type='number'
                  onChange={(event) => handleChange(event, selectionId)}
                  value={stake}
            ...
            {stake > 0 && (
              <div className='mt-3 border-top pt-2'>
                <small>
                  Potential Return:{' '}
                  <strong>${parseFloat(stake * odds).toFixed(2)}</strong>
                </small>
              </div>
            )}
          </Container>
      ...
      <div className='d-flex mt-2'>
        <Button
          ...
          disabled={isPlaced || totalStake === 0}
        >
          Place a bet
        </Button>
        <Button
